<div class="md:grid md:grid-cols-3 md:gap-6">
        ...
        <form wire:submit.prevent="save">
            <div
                ...
                <div class="grid grid-cols-6 gap-6">
                    @if (session()->has('message'))
                        <div
                            class="col-span-6 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400"
                            role="alert">
            ...
            <div
                ...
                <button type="submit"
                        ...
                        class="{{
    !$this->changed ?
    'text-white bg-gray-400 dark:bg-gray-500 cursor-not-allowed font-medium rounded-lg text-sm px-5 py-2.5 text-center' :
    ...
    @if ($isModalOpen)
        <div class="
             inset-0 bg-black bg-opacity-40 flex
             overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full
             ">
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                <div
                    class="relative bg-white rounded-lg shadow   dark:bg-gray-800 sm:rounded-tl-md sm:rounded-tr-md">
                    ...
                    <div
                        class="flex w-full justify-end items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                        <button type="button"
                                wire:click="closeCityModal"
                                class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            {{__('Cancel')}}
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            Livewire.on('modalOpen', () => {
                setTimeout(() => {
                    const inputField = document.getElementById('search-city-name-input');
                    if (inputField) {
                        inputField.focus();
                    }
                }, 50);
